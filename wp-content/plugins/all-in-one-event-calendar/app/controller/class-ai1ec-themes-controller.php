  ...
  public function active_template_url() {
	...
	function update_core_themes() {
		...
		if ( $active_version < 5 ) {
			...
			$folders_to_make[] = $dest_dir . "vortex/js/pages";
		}
		...
		if( count( $files ) > 0 ||
			...
			foreach ( $folders_to_make as $folder_to_make ) {
				...
				if( FALSE === $wp_filesystem->mkdir( $folder_to_make ) ) {
		...
		// TODO: only update the theme version when the update was successfull
		// otherwise provide a way for the user to review the error log that this
		// update generated and to run the update again, after fixing the reported errors
